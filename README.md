...
- **第8步测试案例口述落表**:4-6步齐后 Agent 邀请"讲一个你会试的场景",口述被 `parseTestCaseStory` 拆成 名称/类型(常规/边界/失败/不适用)/输入/预期/失败原因 后**追加**进第8步表格(绝不覆盖已有);预期没说清会追问补上;≥5例且覆盖齐→引导去跑预检;讲不出来随时可去表格填,不强迫;
...
19. ✅ lint / typecheck / test(84例) / build / Playwright E2E 全部通过
20. ✅ 本README支持10分钟启动
