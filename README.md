...
**表单不再是主入口——语言才是。** 每个项目只有一个通用聊天框(`/projects/[id]/chat`):Agent 以面试官身份一次问一个问题,参赛者用口语回答,系统实时把回答提取成结构化材料字段(承诺/披露/赛道/第4-6步全部字段),信卡下方以"✓ 已记录"胶囊即时反馈,右侧记分牌(进度环/任务条/段位)同步跳动。原 10 步向导降级为**结构视图**(查看/批量修改/第8步测试表格/预检),从对话页一键直达。

- **离线大脑**(`lib/llm/chat-brain.ts`):面试状态机——一次只问一个空位、答非所问时语义路由拨正(问场景答频率→存入frequency)、发现漏洞即插入拷问;9个单测覆盖;
- **GLM 模式**:提示词驱动自然语言提取(sanitize 只信白名单字段),解析失败自动回落离线大脑;
