...
export const STEPS: StepConfig[] = [
  ...
  },
  {
    ...
    slug: "problem",
    title: "描述真问题",
    subtitle: "写清楚谁在什么场景下、多长时间遇到一次什么麻烦。空泛问题和伪需求会被识别。",
    minutes: 15,
    ...
    fields: [
      ...
      { key: "worstStep", label: "最麻烦或最容易出错的一步", type: "textarea", required: true, rows: 2, placeholder: "例:两系统字段名不一致,人工对错行后返工" },
      { key: "currentCost", label: "当前时间成本或质量问题", type: "text", required: true, placeholder: "例:每周约2小时,且每月约1次错漏" },
  ...
  },
  {
    ...
    slug: "requirements",
    title: "需求挖掘与判定标准",
    subtitle: "需求没有挖清楚,不要急着开始自动化。先写清什么算可用、什么不可接受、依据来自哪里。",
    minutes: 20,
    ...
    fields: [
      ...
      { key: "judgmentSource", label: "判定依据来自哪里", type: "textarea", required: true, rows: 2, placeholder: "例:以两系统导出的原始记录为准,字段对照表见附件", hint: "求证闭环字段:检查环节的依据必须明确" },
      ...
      { key: "stopConditions", label: "哪些异常必须停止并交给人", type: "textarea", required: true, rows: 2, placeholder: "例:数据行数对不上、字段缺失、疑似非公开信息", hint: "求证闭环字段:异常停止条件" },
  ...
  },
  {
    ...
    slug: "mvp",
    title: "MVP、人机边界与工具链",
    subtitle: "一句话MVP、一个核心闭环、一个可验证指标。主动砍掉平台化和非必要功能。",
    minutes: 20,
    ...
    fields: [
      ...
      { key: "coreLoop", label: "一个核心闭环(输入→处理→检查→人工确认→输出)", type: "textarea", required: true, rows: 2, placeholder: "例:两份CSV→自动对齐合并→按字段对照表检查→人工确认→输出说明" },
      ...
      { key: "tools", label: "使用什么工具", type: "textarea", required: true, rows: 2, placeholder: "例:GLM API + Python脚本 + 表格软件,无其他依赖" },
      { key: "notDoing", label: "本期明确不做什么", type: "textarea", required: true, rows: 2, placeholder: "例:不做多系统直连、不做自动发送、不做移动端" },
    ],
  },
  {
    step: 7,
...
export const TEAM_FIELDS: StepField[] = [
  ...
  { key: "externalResources", label: "使用的开源项目、模板、模型和外部资源", type: "textarea", required: true, rows: 2, placeholder: "例:GLM API、WorkBuddy、某开源表格库(须如实披露)" },
