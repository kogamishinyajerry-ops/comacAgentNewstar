...
export default async function OrganizerDashboard() {
  ...
  const stats = [
    { label: "作品总数", value: projects.length },
    { label: "队伍", value: teams.length },
    { label: "参与者", value: users.filter((u) => u.role === "PARTICIPANT").length },
    { label: "已提交", value: byStatus("SUBMITTED") + byStatus("PRELIMINARY") + byStatus("FINAL") },
    { label: "退回补充", value: byStatus("RETURNED") },
    { label: "高风险标记", value: highRisks.length, tone: highRisks.length ? "red" : "gray" },
  ];
    ...
    <div className="space-y-5 py-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          ...
          <a href="/api/organizer/export?type=projects" className="rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs text-slate-700 hover:bg-slate-100">导出名单</a>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-6">
        {stats.map((s) => (
          <div key={s.label} className="rounded-lg border border-slate-200 bg-white p-3 text-center">
            <p className={`text-2xl font-bold ${s.tone === "red" ? "text-red-600" : "text-slate-800"}`}>{s.value}</p>
